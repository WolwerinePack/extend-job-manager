...
class tz_ad600_widget extends WP_Widget {
	...
	function TZ_Ad600_Widget() {
		...
		/* Widget settings */
		$widget_ops = array( 'classname' => 'tz_ad300_widget', 'description' => esc_html('A widget that allows the display and configuration of of a single 160x600 Banner.', THEME_NAME) );

		/* Widget control settings */
		$control_ops = array( 'width' => 160, 'height' => 600, 'id_base' => 'tz_ad600_widget' );

		/* Create the widget */
		$this->__construct( 'tz_ad600_widget', esc_html(THEME_FULL_NAME.' Custom 160x600 Ad', THEME_NAME), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="banner">';
			...
			if($contactID) {
				//echo '<a href="'.get_page_link($contactID[0]).'" class="banner-meta">'.esc_html("Contact us about advert spaces", THEME_NAME).'<i class="fa fa-angle-double-up"></i></a>';
			}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e('Title:', THEME_NAME) ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" />
		</p>
		 -->
		<!-- Ad image url: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'ad' ); ?>"><?php esc_html_e('Ad image url:', THEME_NAME) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php esc_html_e('Ad link url:', THEME_NAME) ?></label>
